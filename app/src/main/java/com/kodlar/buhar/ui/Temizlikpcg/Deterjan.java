    ...
    public void onStart(){
        ...
        FirebaseRecyclerOptions options=
                new FirebaseRecyclerOptions.Builder<Urun1>()
                        .setQuery(DeterjanRef, Urun1.class)
                        .build();

        final FirebaseRecyclerAdapter<Urun1, DeterjanViewHolder> adapter = new FirebaseRecyclerAdapter<Urun1, DeterjanViewHolder>(options) {
            @Override
            protected void onBindViewHolder(@NonNull final DeterjanViewHolder DeterjanViewHolder, int i, @NonNull Urun1 urun) {
                ...
                DeterjanRef.child(userIDs).addValueEventListener(new ValueEventListener(){
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot)
                    {
                        ...
                        DeterjanViewHolder.sepetArti.setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                ...
                                DeterjanViewHolder.miktar++;

                                SepetRef.child(userIDs).setValue(new Urun1(urunadi, urunagirlik, urunfiyat, urunfotografi, urunid));
                                SepetRef.child(userIDs).child("miktar").setValue(DeterjanViewHolder.miktar);
                                SepetRef.child(userIDs).child("uruntutari").setValue(urun.getUruntutari());
                                Snackbar.make(DeterjanView, "Sepetinize ürün eklendi.", Snackbar.LENGTH_LONG)
                                        .setAction("Action", null).show();


                                urun.setUruntutari(DeterjanViewHolder.miktar * urunfiyat);
